...
repository. We'll read them in now. (They were exported from REDCap directly
using the `CSV + labels` option; with this option, the field values as well as
the field names use the labeled text (eg, "Date of birth") vs the field names
("dob"). Therefore, one step will be to rename the fields in each database with
actual variable names.) I've also stored a CSV of the data dictionary in the
same folder.

```{r csv_data}
## -- Get variable names for each form -----------------------------------------
## 1. Read in data dictionary, give it names manually (we'll also use it later)
datadict_names <- c(
...
datadict <- read.csv("rawdata/datadict.csv", stringsAsFactors = FALSE)
names(datadict) <- datadict_names

## 2. Use data dictionary to get names for all the other forms
demog_names <- c(
  subset(datadict, form_name == "demographics")$field_name,
  "demographics_complete"
)
## Take out warning "variables"
demog_names <- setdiff(demog_names, "age_warning")
## Handle matrices & checkboxes <shakes fist>
demog_names <- unlist(
  lapply(
    as.list(demog_names),
    function(x){
      if(x %in% c("gym", "aerobics", "eat", "drink")){
        paste(x, 0:6, sep = "_")
      } else if(x == "which_statins"){
        paste(x, 1:9, sep = "_")
      } else if(x == "study_id"){
        c(x, "redcap_event_name")
      } else{
        x
      }
    }
...
## Shoutout to this brilliant person on StackOverflow:
## https://stackoverflow.com/a/48768548/7086401

baseline_names <- c(
  subset(datadict, form_name == "baseline_data")$field_name,
  "baseline_data_complete"
)
monthly_names <- c(
  setdiff(
    subset(datadict, form_name == "monthly_data")$field_name, "consent_reminder"
  ),
  "monthly_data_complete"
)
completion_names <- c(
  subset(datadict, form_name == "completion_data")$field_name,
  "completion_data_complete"
)

baseline_df <- read.csv(
  "rawdata/baseline_df.csv",
  stringsAsFactors = FALSE,
  na.strings = ""
)
names(baseline_df) <- c(demog_names, baseline_names)

monthly_df <- read.csv(
  "rawdata/monthly_df.csv",
  stringsAsFactors = FALSE,
  na.strings = ""
)
names(monthly_df) <- c("study_id", "redcap_event_name", monthly_names)

completion_df <- read.csv(
  "rawdata/completion_df.csv",
  stringsAsFactors = FALSE,
  na.strings = ""
)
names(completion_df) <- c("study_id", "redcap_event_name", completion_names)
...
## Versions if using CSV format: options given download checkboxes as `Checked`
##  or `Unchecked`
demog_issues[, "no_activity"] <- rowSums(
...
Notes: this should clear up a lot of queries, but not baseline ones

- Entering an consent date for patient 3

Notes: this should *introduce* a new query, because the patient's DOB is in 2018 and thus age will be, like, one month

- Noting that we forgot to weigh patient 3 at baseline; thus that value is
unfixable (query 3_2018-05-12_2)

Notes: This one shouldn't show up in next clean
...
Doing this manually does mean that some changes to the data are not documented.
We have found this to be a fine tradeoff for the number of hours saved
documenting the same issue and resolution, especially given that one can always
access data histories for a given field within REDCap itself.
