...
We need to do a bit of data management: We need dates of birth and consent to be
`Date` formats so that we can compare them.

```{r demog_datamgmt}
baseline_df$dob <- as.Date(baseline_df$dob, format = "%Y-%m-%d")
baseline_df$date_enrolled <- as.Date(
  baseline_df$date_enrolled, format = "%Y-%m-%d"
)

```

### 3. Custom error checking (+ demo of the process)
...
rownames(demog_issues) <- paste(
  baseline_df$study_id, "Baseline Visit", sep = '; '
)
...
demog_issues[
  with(baseline_df, {
    difftime(date_enrolled, dob, units = "days") < (18 * 365.25) |
      difftime(date_enrolled, dob, units = "days") > (110 * 365.25)
  }),
  "dob_limits"
] <- TRUE
...
## Birth questions
demog_issues[
  with(baseline_df, !is.na(gender) & gender == "Female" & is.na(given_birth)),
  "birth_yn"
] <- TRUE
demog_issues[
  with(baseline_df, {
...
demog_issues[
  rowSums(
    !is.na(
      baseline_df[
        ,
        grep("^(gym|aerobics|eat|drink)\\_[0-6]$", names(baseline_df))
        ]
    )
  ) == 0,
  "no_activity"
...
## Statins
demog_issues[
  baseline_df$any_statins == "Yes" &
    rowSums(!is.na(baseline_df[, paste0("which_statins_", 1:9)])) == 0,
  "which_statin"
] <- TRUE
demog_issues[
  baseline_df$any_statins == "No" &
    rowSums(!is.na(baseline_df[, paste0("which_statins_", 1:9)])) > 0,
  "no_statins"
] <- TRUE
...
(Variable labels can be very helpful in error messages! No matter how descriptive your field names are, your data entry team is used to seeing "Subject Global
Assessment," not "sga_b". With little effort, you can make their lives much
easier and also make the data cleaning process much faster. Here, the data
dictonary is imported in `dataclean_helpers.R`, using a process similar to the
one we used to download our raw data, and stored as the object `datadict`.)
...
Perfect. That's all we want to check for the Demographics form, so we'll combine
those two data.frames into a single one for ease.

```{r demog_combine}
demog_final <- do.call(rbind, list(demog_missing, demog_errors))
