...
baseline_df <- merge(
  subset(dummy_df, redcap_event_name == "Baseline Visit", select = c(study_id)),
  baseline_df,
  by = "study_id", all.x = TRUE
)
...
completion_df <- merge(
  subset(dummy_df,
         ...
         select = c(study_id, date_enrolled)),
  completion_df,
  by = "study_id", all.x = TRUE
)
...
# monthly_df$days_enroll_planned <- with(monthly_df, {
...
monthly_df$out_of_study <- with(monthly_df, {
  (!is.na(death_date) & death_date < days_enroll_planned) |
    (!is.na(studywd_date) & studywd_date < days_enroll_planned)
})
...
- % compliance present
- # treatments should be between 0-8 and “match” % compliance
