...
subtitle: "Additional resources on Github: [github.com/jenniferthompson/DataCleanExample](https://github.com/jenniferthompson/DataCleanExample)"
...
- The tutorial was originally written for a workshop at Osaka City University,
June 2018, and thus refers to OCU's REDCap instance and example databases stored
there.

    While the primary information and code needed for the data cleaning process
    will be within this PDF, all code and files related to this tutorial are
    public and stored in [this Github
    repository](https://github.com/jenniferthompson/DataCleanExample), which
    can be downloaded as a zip file. If you wish to work through this tutorial
    without access to OCU's REDCap instance, identical toy data is in the
    repository; the correct files to use will be noted throughout.
    
    The file [`dataclean_script.R`](https://github.com/jenniferthompson/DataCleanExample/blob/master/dataclean_script.R) in the Github repository is a
    [purled](https://www.rdocumentation.org/packages/knitr/versions/1.20/topics/knit)
    version of the code in this PDF, and will be useful to anyone trying to
    follow along (OCU or not!).

- All code for data export assumes that the user has rights to use the REDCap
API for data export, and that a working API token is stored in the `.Renviron`
file in the working directory, in the format

    `TOKENNAME=manylettersandnumbers`
    
    If you don't want to use the `.Renviron` file, you can replace
    `Sys.getenv("TOKENNAME")` with `"manylettersandnumbers"`. If you do this,
    do not share your code - it is very important to keep your API token
    secret.
...
![Example study data collection](images/example_events.png)
...
investigate and debug potential issues or add code for new checks. The following things are very helpful during this process:

- Agreement that the data clean is a top priority: This doesn't necessarily
mean that it is everyone's most *urgent* priority, but it does mean that
everyone agrees that this is important and should have designated time at an
appropriate date.
...
## Overview of R Script Structure

The rest of this tutorial will focus on the steps of building the R script. In
general, my process for the script looks like this:

1. Download data from REDCap using the API (Chapter 3)
1. Do any needed data management
1. For each form (Chapter 4):
    1. Set up
    1. Check for simple missingness (eg, date of birth should always be present in the baseline form)
    1. Check for data with unlikely values (eg, creatinine that is unexpectedly high or low)
    1. Other, more complicated checks
1. Combine issues from all forms into a single data.frame
1. Remove issues that were previously documented as "unfixable" or correct in
the documentation database (Chapter 5)
1. Prepare issues to disseminate to study staff (Chapter 6)
...
You can then access the token using the function `Sys.getenv()`. For the
purposes of this tutorial, mine looks like Figure 3.2 (with actual tokens grayed
out).

![.Renviron example](images/renviron_example.png)
...
This tells R what REDCap instance to connect to. It will look something like this:

`https://redcap.vanderbilt.edu/api/`

`https://redcap.med.osaka-cu.ac.jp/redcap/api/`

If you don't know your API URL, you can find it this way once you have your API token (thank you to [Benjamin Nutter, author of the redcapAPI package!](https://github.com/nutterb/redcapAPI/wiki/Finding-your-REDCap-API-URL)):

1. Go to the API documentation of a project for which you have a working API token
1. Choose an option under "Supported Actions," such as "Export Records"
1. The URL will appear on the next page

Once you have your API URL, make sure to update it in your copies of
`dataclean_script.R` and `dataclean_helpers.R` in order to continue the tutorial.

### 3. R's `httr` package, built for working with APIs
...
this tutorial is intentionally in base R.)

## Approaches

There are (at least) two approaches to exporting REDCap data:

1. Read in the entire database in a single, relatively simple `httr::POST` call,
then create subsets in R as needed
1. Read in specific subsets in separate, more complex `httr::POST` calls

Approach #1 is fine if your database is not longitudinal, complex, or very
large, and/or if you are not yet very comfortable working with the API. Approach #2 is valuable in more complicated situations.

For example, here, our database is longitudinal, and different data is
collected at different time points. If we read in the entire database at once,
we will have a lot of missing values - for example, date of birth will be
missing in each row representing monthly visits and study completion. We can
fairly easily create subsets of the data for each event, but it can be annoying
to subset only the *columns* needed for each event, either manually or using the
data dictionary.
...
## post_to_df() is mostly a wrapper for read.csv() with our preferred options;
##  it also removes repeated underscores from variable names
...
data.frames will have many columns with blank values, because our study is
longitudinal (can have multiple events with different data collected at each).
For example, `baseline_df` will have a column for `compliance`, even though
`compliance` is only collected at monthly visits. This is not a major problem if
your project is small, but can be a problem if you have a large project.
...
in the `body` argument of `httr::POST`. *(Note that as of the time of this writing, the example R code from the Playground uses `RCurl`; `httr` is
currently more commonly used, and thus it is easier to find documentation and
assistance for it, so I have used it here.)*
...
REDCap project, and raw form names by either looking at the data dictionary or downloading the instrument mappings from `Project Setup -> Designate Instruments
for My Events`. (They can also be exported as project metadata using the API;
there is an example in `dataclean_helpers.R`.)

**Note:** The helper function `post_to_df()`, which creates a data.frame from
the result of `httr::POST`, is created in `dataclean_helpers.R`. It has two
steps:

1. Read in a `POST` object as a CSV, with our preferred options for `read.csv()`.
1. Remove repeated `_` characters from variable names.
...
Monthly Visit form), as well as study ID. It exports all three monthly visit events; all other events would have `NA` values for these fields. Each study ID
...
**OCU team**: If your API tokens are set up and stored in your `.Renviron` file
like mine, you should be able to run all the code up to this point. If you don't
want to use the `.Renviron` file, you can run all the code but replace `token = Sys.getenv("RCTOKEN_OCU_ORG")` with `token = "yourapitoken"`.

If you do not have an API token for OCU's REDCap instance, I've stored some toy
data in the same format as our example REDCap databases, along with a copy of
the data dictionary, in the `rawdata/` folder in this repository to allow you to
continue with the tutorial. All are in the same format we would see if we used
the API as described above.

If you have downloaded the entire repository, you can run this code:
...
into our documentation database(s) (see Chapter 6) so that the issues can be
resolved.
...
We often have groups of very similar potential problems: There are many fields that should be present no matter what, and there are several fields which should
be between specific limits. I have written helper functions that check for these
specific types of issues; they are in `dataclean_helpers.R`, and we'll see their
use later.

**Remember**: REDCap has many data validation capabilities built in - **use
them!** If your database already checks fields like email for formatting, or
gives live warnings if you enter unlikely lab values, it will save you and the
study staff time and headaches later. The same applies to using branching logic
often and well. The added value for this data cleaning script comes from the
more complex data checks that are possible here and not within REDCap itself.
But the more of REDCap's powerful features you can use from the beginning, the
better your data will be throughout the study (eg, for DSMB reports and interim
analyses).
...
and detail are key.** Typically, our clinical project manager will spend a lot
of time creating a list of things that need to be checked, based on the study
...
![How `create_error_df()` works](images/create_error_df.png)
...
"Subject Global Assessment," not "sga_b". With little effort, you can use the
data dictionary to make their lives much easier and also make the data cleaning
process much faster. Here, the data dictonary is imported in
`dataclean_helpers.R`, using a process similar to the one we used to download
our raw data, and stored as the object `datadict`.)
...
# Step 3: Remove Unfixable/Correct Queries
...
In addition, we may have extreme values in our data which we flag as potential
issues, but are actually correct (eg, someone really did have glucose that
high). These, too, should not be repeated once they are checked and confirmed.
...
For the purposes of this tutorial, we're now going to assume that this is *not*
the first time we've run this script - ie, that there are already a few issues
...
- Correcting a highly implausible creatinine value at baseline for patient 4A
(query 4A_2018-05-12_7; changed from 95 to 9.5). *This will show up in the next
clean, but as a "please confirm that this is accurate" query, because the
creatinine value is still above the suggested maximum.*
...
Everything else should show up again because it hasn't been fixed. This means
that if study staff have not corrected and documented a query before the data
clean is run again, the same issue will show up multiple times. In rare cases
this could be a helpful motivator, but in general, it is **very important** for
the statistics/data team to communicate clearly with clinical/data entry teams
about appropriate timelines for rerunning the script, to avoid unnecessary
double-entry.

## Pause: Download "Fixed" Data

To continue with the tutorial, we need to replace our three datasets with
versions that *include* the corrections I detailed above.

**OCU team:** If your `.Renviron` is set up like mine, you should be able to run
`dataclean_partial_ocu.R` to get to this point in the tutorial, but with the
corrected data rather than the original data.

**Those using CSV files from repository**: The files with corrected data are
stored in the `fixeddata/` directory. The script `dataclean_partial.R` in this
repository reads in these files and runs our data cleaning code thus far on
these three updated datasets, resulting in the file
`querydata/updated_issues.csv`.

If we compare our *new* issues to our *original* ones (in the repository, `querydata/updated_issues.csv` to `querydata/original_issues.csv`), we see that:
...
API token. Mine is saved in my `.Renviron` file as the object `DOCTOKEN_OCU`.

**OCU team**: If your `.Renviron` is set up like mine, you should be able to run
this code chunk to export the documentation data:
...
**Those using the repository**: So that you can continue the tutorial, I've
saved the result of the above in the file `querydata/doc_df.csv`; read it in
using:
...
## Removing Unneeded Issues

We now need to merge our current issues (stored in `all_issues`) with our
previously documented issues and remove the potential errors which are either
unfixable or actually correct. To be removed from future data cleans, queries
must be marked as either:

1. **Accurate**, by the data team in the field `corrected`
1. Confirmed by the coordinating center as being **unfixable** in the field
`cc_conclusion`

We'll create an indicator variable for whether each query meets either condition.
...
Now we need to 1) add that indicator variable onto our new set of data issues,
then 2) remove those two which are confirmed correct or unable to be corrected.
...
**Note**: In `dataclean_script.R`, we just subset `all_issues` directly, without
the intermediate object, because there is no need to keep the entire set of
issues. I use an intermediate object here so that we can more easily compare the
files in the repository, but in your actual workflow it is up to you!

Compared to our original set of queries, we see some new ones; for example, now
that patient 3 has a consent date entered, some new queries are showing up
related to it. We also have two queries with `remove = TRUE` - these are the two
queries listed above, which still meet our definition of potential problems, but
are accurate or cannot be fixed. Let's remove them and we'll be done!
...
using the REDCap API, our team has found it helpful to do this step **manually**.
It is often useful for our clinical coordinators to look over the list prior to
...
Therefore, we typically separate issues by study site (if the study is
multicenter); send these in separate CSV files to the clinical project managers
to look over; and then either upload directly, or wait until the coordinators
have resolved systematic errors with study staff and rerun the data clean once
...
especially given that one can always access data histories for a given field
within REDCap itself.

## Code for API Import

If you do want to import your updated issues automatically using the API, you
...
httr::POST(
 url = rc_url,
 body = list(
   token = Sys.getenv("DOCTOKEN_OCU"), ## token stored in .Renviron
   content = "record",
   format = "csv",
   type = "flat",
...
Once the data is uploaded to the documentation database, the staff and
coordinators work to resolve and document each issue, leaving the database ready
...
there is a piece of data that is systematically showing up as an issue, perhaps
it is due to something that was not clearly addressed at the study startup visit
and needs to be revisited, or should be written out fully in an SOP.

# Conclusions & Acknowledgements

Our group recently finished data collection on the first clinical trial we
conducted using this process - a large, multicenter trial with a complicated
study design. I can say with absolute certainty that we were able to finish data
cleaning and lock our database very quickly, with strong confidence in our
data's accuracy, because we prioritized this data cleaning process throughout
...
Particularly, **Brenda Pun, Cayce Strength, Leanne Boehm, and Aimee Hoskins**
have played huge parts in making this process a reality, in their roles as
project managers and study coordinators.

**Cole Beck and Jeremy Stephens** have contributed ideas and R code throughout
the time that we have used this process, making our scripts more efficient.
...
downloading data and checking problems of similar types easier to do and debug.
When using the workflow described here, this file should be stored in the same
working directory and sourced within the script, as seen above. It is copied
